...
function isEnoughCapacity(products, containerSize) {
  ...
  // Use Object.values() to get an array of the object's values and sum them up
  for (const quantity of Object.values(products)) {
    totalItems += quantity; // Add the quantity to totalItems
  }
  return totalItems <= containerSize; // Simplified the if statement
  //  totalItems <= containerSize is a comparison

  // It evaluates to either:

  // true → if totalItems is less than or equal to containerSize

  // false → if totalItems is greater than containerSize

  // The return statement returns that boolean value directly
}
